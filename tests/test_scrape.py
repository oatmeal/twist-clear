...
class TestFetchHistory:
    def test_empty_date_range_makes_no_api_calls(self, conn, mock_api):
        dt = datetime(2024, 1, 1, tzinfo=UTC)
        total, max_created_at = fetch_history(mock_api, conn, "123", dt, dt)
        assert total == 0
        assert max_created_at is None
        mock_api.get_clips_window.assert_not_called()

    def test_single_empty_window_saves_progress(self, conn, mock_api):
        mock_api.get_clips_window.return_value = ([], False)
        from_dt = datetime(2024, 1, 1, tzinfo=UTC)
        to_dt = datetime(2024, 1, 2, tzinfo=UTC)

        fetch_history(mock_api, conn, "123", from_dt, to_dt)
    ...
    def test_clips_are_stored_in_db(self, conn, mock_api):
        mock_api.get_clips_window.return_value = ([make_api_clip()], False)
        from_dt = datetime(2024, 1, 1, tzinfo=UTC)
        to_dt = datetime(2024, 1, 2, tzinfo=UTC)

        total, _ = fetch_history(mock_api, conn, "123", from_dt, to_dt)

        assert total == 1
        assert conn.execute("SELECT id FROM clips WHERE id = 'clip1'").fetchone() is not None

    def test_returns_max_created_at_across_windows(self, conn, mock_api):
        clips = [
        ...
        _, max_created_at = fetch_history(mock_api, conn, "123", from_dt, to_dt)

        assert max_created_at == "2024-01-01T20:00:00Z"

    def test_window_narrows_when_has_more(self, conn, mock_api):
    ...
    def test_progress_saved_after_every_completed_window(self, conn, mock_api):
    ...
    def test_min_window_guard_does_not_loop(self, conn, mock_api):
    ...
    def test_window_grows_after_successful_windows(self, conn, mock_api):
    ...
    def test_no_clips_returns_none_for_max_created_at(self, conn, mock_api):
        mock_api.get_clips_window.return_value = ([], False)
        from_dt = datetime(2024, 1, 1, tzinfo=UTC)
        to_dt = datetime(2024, 1, 2, tzinfo=UTC)

        total, max_created_at = fetch_history(mock_api, conn, "123", from_dt, to_dt)
